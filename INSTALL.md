...
- `PyYAML>=6,<7`
- `sclite-core>=0.7.0a0,<0.8` (PyPI distribution; Python import package `sclite`)
...
python3 -m venv .venv
source .venv/bin/activate
pip install ravenclaw-security==0.17.0
python - <<'PY'
import ravenclaw
...
The `0.17.0` wheel is intentionally a narrow public contract/profile package.
...
python3 -m venv .venv
source .venv/bin/activate
pip install PyYAML "sclite-core>=0.7.0a0,<0.8" "govengine>=0.10.2a0,<0.11"
python scripts/validate_public_install.py
```
...
pip install PyYAML "Flask>=3,<4" "pytest>=8,<9" \
  "sclite-core>=0.7.0a0,<0.8" "govengine>=0.10.2a0,<0.11"
python scripts/validate_public_install.py --dev
```
