...
def test_scope_fidelity_report_passes_exact_host_binding() -> None:
    ...
    assert report['target_host_match_status'] == 'exact'
    assert report['request_shape_hygiene_status'] == 'clean'
    assert report['mismatched_hosts_detected'] == []
...
def test_scope_fidelity_report_fails_cross_host_drift() -> None:
    ...
    assert report['target_host_match_status'] == 'mixed'
    assert report['request_shape_hygiene_status'] == 'cross_host_mismatch'
    assert report['mismatched_hosts_detected'] == ['evil.example', 'static.example.net']
    assert report['request_shape_hygiene_source'] == 'normalized_args+execution_plan'
...
def test_scope_fidelity_report_marks_missing_hosts_for_review() -> None:
    ...
    assert report['target_host_match_status'] == 'none_detected'
    assert report['request_shape_hygiene_status'] == 'ambiguous'
