...
- the first ownership-narrowing result: Ravenclaw executes security policy and scope decisions through `engine/security_policy_gateway.py` using its host-owned scope state; `govengine.policy.gateway` remains an upstream compatibility surface, not Ravenclaw runtime authority
