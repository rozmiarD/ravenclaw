...
- Consolidated the `debug` agent onto the canonical workspace by repointing its workspace to `/home/probo/.openclaw/workspace` while keeping its explicit no-fallback `gpt-5.4` model configuration.
...
- Execution engine PATH now includes `/home/probo/.local/bin` so userland-installed tooling can be executed without system-wide package installs.
...
- Agentic Roles: model names now sourced from /home/probo/.openclaw/openclaw.json; status/LED mapping improved (run_state fallback).
