...
## Unreleased
- Added a Ravenclaw compatibility entrypoint for GovEngine's optional security-profile facade so Ravenclaw can consume `govengine.security_profile` when available while retaining a fallback for the published `govengine==0.1.4` line; public install validation now reports that compatibility seam.
