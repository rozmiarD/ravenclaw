...
def test_public_truth_validator_passes() -> None:
    ...
    assert proc.stdout.strip().startswith('public_truth_ok:ravenclaw-security==0.18.3:govengine>=0.12.2a0,<0.13:')
...
def test_public_truth_validator_rejects_upstream_gateway_as_active_architecture() -> None:
    ...
    assert errors == [
        ...
        'ARCHITECTURE.md:missing_host_owned_action_tooling_claim:engine/security_signal_contract.py',
        'ARCHITECTURE.md:missing_host_owned_action_tooling_claim:engine/security_analysis_contract.py',
        'ARCHITECTURE.md:missing_host_owned_action_tooling_claim:engine/security_evidence_policy.py',
    ]
...
def test_public_truth_validator_rejects_upstream_security_review_helpers_as_active_architecture() -> None:
    validator = _load_validator()
    ...
    assert 'ARCHITECTURE.md:upstream_security_review_helper_listed_as_active_main_file' in errors
