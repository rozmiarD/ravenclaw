...
| `EvidenceBundle` | Public-safe demo evidence bundle schema-backed in v0.1; broader qualification schema later | evidence and qualification modules such as `engine/vuln_qualification.py` plus GovEngine confirmation-evidence policy helpers; public artifact via SCLite and `engine/security_contract_layer.py` compatibility wrapper | reporting/follow-up/public proof | Schema: `schemas/evidence_bundle.v0.1.schema.json`; reference: `references/evidence-bundle-v0.1.md`; states dry-run proof criteria and non-claims. |
