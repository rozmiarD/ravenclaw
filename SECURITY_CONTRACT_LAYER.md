...
Ravenclaw also keeps `engine/govengine_boundary_profile.py` as a thin consumer for GovEngine's kernel/profile boundary report. Public install validation now requires the package chain, `govengine>=0.12.1a1,<0.13` and `sclite-core>=0.8.0b2,<0.9`, to expose `govengine.kernel_boundary_report`, the Domain Profile SDK, and runtime contract proof surfaces, and reports failure if the boundary-profile check does not pass. The range consumes the published neutral-only GovEngine `0.12.1a1` line.
