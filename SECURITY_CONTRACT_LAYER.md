...
Ravenclaw also keeps `engine/govengine_boundary_profile.py` as a thin consumer for GovEngine's kernel/profile boundary report. Public install validation now requires the transitional package chain, `govengine>=0.11.0a0,<0.13` and `sclite-core>=0.8.0a0,<0.9`, to expose `govengine.kernel_boundary_report`, the Domain Profile SDK, and runtime contract proof surfaces, and reports failure if the boundary-profile check does not pass. The range accepts published `0.11.0a0` and clean local validation of the neutral-only `0.12.0a0` candidate; publication sync is separate.
