...
Ravenclaw also keeps `engine/govengine_boundary_profile.py` as a thin consumer for GovEngine's kernel/profile boundary report. Public install validation now requires the package chain, `govengine>=0.14.0,<0.15` and `sclite-core>=1.0.3,<1.1`, to expose `govengine.kernel_boundary_report`, the Domain Profile SDK, and runtime contract proof surfaces, and reports failure if the boundary-profile check does not pass. The range consumes the published GovEngine `0.14.0` line.
