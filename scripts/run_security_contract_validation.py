...
def _demo_bundle_check(output_dir: Path, demo_repo: Path) -> ValidationCheck:
    return ValidationCheck(
        ...
        cwd=ROOT,
        env={'DEMO_REPO': str(demo_repo), 'DEMO_OUTPUT': str(output_dir), 'PYTHON_BIN': sys.executable},
    )
...
def _proof_of_value_scorecard_check(snapshot_dir: Path) -> ValidationCheck:
    return ValidationCheck(
        check_id='proof_of_value_scorecard',
        description='Build and validate the public-safe proof-of-value scorecard from copied snapshot evidence paths.',
        command=[sys.executable, 'scripts/build_proof_of_value_scorecard.py', '.', '--check'],
        cwd=snapshot_dir,
    )
...
def _github_actions_pytest_matrix_check(matrix_repo: Path) -> ValidationCheck:
    return ValidationCheck(
        ...
        command=[
            'bash',
            '-lc',
            'scripts/assemble_public_snapshot.sh "$MATRIX_REPO" >/dev/null && cd "$MATRIX_REPO" && for slice in $MATRIX_SLICES; do echo "== $slice =="; "$PYTHON_BIN" scripts/run_pytest_slice.py "$slice"; done',
        ],
        cwd=ROOT,
        env={'MATRIX_REPO': str(matrix_repo), 'MATRIX_SLICES': ' '.join(GITHUB_ACTIONS_PYTEST_SLICES), 'PYTHON_BIN': sys.executable},
    )
